...
<h3 style="color: #2563eb;">驱动</h3>

<blockquote style="border-left: 4px solid #2563eb; padding: 12px 16px; background: #f0f4ff; margin: 12px 0; border-radius: 0 4px 4px 0;">
💡 <strong>一句话总结</strong>：驱动是操作系统和硬件之间的翻译官——OS 不认识硬件，驱动把硬件翻译成 OS 能理解的标准接口，每个硬件必须有驱动才能用。
</blockquote>

### 是什么

操作系统不直接认识硬件——摄像头、NPU、CAN 控制器各有各的控制方式。驱动就是把这些硬件的控制方式翻译成 OS 的标准接口。
...
OS : "发给文件系统驱动"
    ↓
驱动: "告诉 NVMe 控制器：读 LBA 0x42, 4KB"
    ↓
SSD 硬件: 执行、返回数据
```

插一个摄像头，系统不认识——因为没人告诉 OS"这个设备怎么用"。**驱动就是那本说明书。**
...
─────────────────        ────────────
摄像头（MIPI CSI）   →   CSI 驱动
NPU（AI 加速器）     →   QNN 驱动 / HTP 驱动
GPU                   →   Adreno 驱动
CAN 控制器            →   SocketCAN 驱动
```

每个硬件必须有驱动，OS 才能用。驱动由芯片厂商（如高通）或 BSP 团队编写。
